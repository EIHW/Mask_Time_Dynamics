...
class PositionalEncoder(nn.Module):
    def __init__(self, max_len, d_model, scale=None):
        ...
        pe = torch.reshape(torch.FloatTensor(pe_flat), (max_len, d_model)).cuda()
...
class CRNN(nn.Module):
    def __init__(self, conv_net='cnn', recurrent_net='lstm', wi_gender=None):
        ...
        self.act = nn.ReLU()
        self.do = nn.Dropout(0.2)   # 0.5
        self.fc2 = nn.Linear(64, 2)
        self.process_lld = ProcessLLD()

        self.wi_gender = wi_gender
        if self.wi_gender:
            self.fc1_gender = nn.Linear(128, 64)  # 128 + 32, 64
            self.act_gender = nn.ReLU()
            self.do_gender = nn.Dropout(0.2)   # 0.5
            self.fc2_gender = nn.Linear(64, 2)
    ...
    def forward(self, x):   # lld
        ...
        # cascade lld output and spectrum output
        # lld_out = self.process_lld(lld)
        # out = torch.cat([out, lld_out], dim=-1)
        embedding = out
        out = self.act(self.fc1(out))
        features = out
        out = self.fc2(self.do(out))
        if self.wi_gender:
            out_gender = self.act_gender(self.fc1_gender(embedding))
            out_gender = self.fc2_gender(self.do_gender(out_gender))
            return out, features, out_gender
        return out, features


if __name__ == '__main__':
    cnn = CRNN(conv_net='cnn', recurrent_net='none')
    dummy = torch.zeros(4, 3, 40, 321)
    output, features_ = cnn(dummy)
